...
$(document).ready(function() {
    ...
    if ($(window).width() > 1139) {
        $('body').on('mouseenter', '.map-russia-district-42-2, .map-region-item-42-2', function(e) {
            $('.monitoring-map-region-hint').remove();
            $('body').append('<div class="monitoring-map-region-hint">' + $(this).attr('data-title') + '</div>');
            var curLeft = e.pageX;
            var curTop = e.pageY;
        ...
        $('body').on('mousemove', '.map-russia-district-42-2, .map-region-item-42-2', function(e) {
            var curLeft = e.pageX;
            var curTop = e.pageY;
            $('.monitoring-map-region-hint').css({'left': curLeft, 'top': curTop});
        });

        $('body').on('mouseleave', '.map-russia-district-42-2, .map-region-item-42-2', function(e) {
            $('.monitoring-map-region-hint').remove();
        });
    }
    ...
    $('body').on('click', '.map-russia-district-42-2', function() {
        ...
        if (!$(this).hasClass('disabled')) {
            ...
            $('.map-region-42-2[data-id="' + curID + '"]').show();
            var curData = [];
            ...
            for (var i = 0; i < curData.length; i++) {
                ...
                if (curColorIndex > 0) {
                    newMap += '<g class="map-region-item-42-2" data-id="' + regionID + '" data-title="' + regionTitle + '">';
                } else {
            ...
            $('.map-region-42-2[data-id="' + curID + '"] .map-region-item-42-2:not(.disabled)').each(function() {
                ...
                var curDiff = curRegion.parents().filter('.map-region-42-2 svg').width() / 1108;

                for (var i = 0; i < face_42_2_dataRegions.length; i++) {
                    if (curRegion.attr('data-id') == face_42_2_dataRegions[i].id) {
                        var curValue = parseInt(face_42_2_dataRegions[i].value.replace(/ /g, ''));
                        for (var j = 0; j < curValue; j++) {
                            pointsHTML += '<div class="map-region-42-2-point" data-region="' + curRegion.attr('data-id') + '" data-id="' + j + '" style="left:' + (curCenter[0] * curDiff) + 'px; top:' + (curCenter[1] * curDiff) + 'px"><div class="map-region-42-2-point-value"></div></div>';
                        }
                    }
                }
                pointsHTML += '</div>';
...
function face42_2_Redraw() {
    ...
    if (curData !== null) {
        ...
        $('.map-russia-district-42-2:not(.disabled)').each(function() {
            ...
            $('.map-russia-42-2-inner').append('<div class="map-russia-42-2-point" style="left:' + (curCenter[0] * curDiff) + 'px; top:' + (curCenter[1] * curDiff) + 'px"><div class="map-russia-42-2-point-value">' + curDistrict.attr('data-value') + '</div></div>');
        });
    }
